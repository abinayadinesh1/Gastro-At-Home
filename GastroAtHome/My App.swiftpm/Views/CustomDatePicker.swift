...
struct CustomDatePicker: View{
    ...
    @State var chartShowing : Bool = false
    ...
    var body: some View{
        ...
        VStack(alignment: .leading, spacing: 20){
            ...
            HStack{
                HStack(spacing: 10){    
                    Text(dateHeader()[0]).bold().font(.system(size: 35)).padding(.bottom,10)
                ...
                }.padding(.top, screenHeight/32).padding(.trailing,25)}.padding(.bottom,10)
                ...
                if let poop = poops.first(where: {poop in 
                    ...
                    ForEach(poop.poop){poop in 
                        VStack(alignment: .leading, spacing: 10){
                            ...
                            Text("Description: \(poop.description)").font(.title3)
                            Text("Rating: \(poop.ratePoop)/10").font(.title3).padding(.bottom, 10)
                            Text("Color: \(poop.color)").font(.title3).padding(.bottom, 10)
                            Text("Size: \(poop.size)").font(.title3).padding(.bottom, 10)
                            Text("Shape: \(poop.shape)").font(.title3).padding(.bottom, 10)
                            Button(action: {
                                self.chartShowing.toggle()
                            }, label: {
//                                Text("View Bristol Meyers Poop Chart").foregroundColor(.blue)
                                Image(systemName: chartShowing == true ? "poopChart" : "")
                                    .resizable()
                            })
